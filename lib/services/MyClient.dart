...
class MyClient extends http.BaseClient {
  ...
  Future<Response> get(url, {Map<String, String> headers}) async {
    ...
    return _httpClient.get(url, headers: headers).timeout(const Duration(seconds: 30));
  }
  ...
  Future<Response> post(url,
      {Map<String, String> headers, body, Encoding encoding}) async {
        try{
    headers.addAll(defaultHeaders);
    return _httpClient.post(url, headers: headers,body:body, encoding: encoding).timeout(const Duration(seconds: 60));
        }finally{
          _httpClient.close();
        }
  }
